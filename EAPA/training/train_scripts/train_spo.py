        ...
        for batch in tqdm(
            ...
            for train_batch_idx in tqdm(
                ...
                with accelerator.accumulate(unet):
                    ...
                    image1 = pipeline.vae.decode(sample_0["latents"]).sample
                    image1 = image1.cpu().numpy()[0]
                    image1 = (image1 * 255).astype(np.uint8)
                    image1 = image1[0]*0.299 + image1[1]*0.587 + image1[2]*0.114
                    image1 = np.expand_dims(image1, axis=0).astype(np.uint8)
                    
                    image2 = pipeline.vae.decode(sample_1["latents"]).sample
                    image2 = image2.cpu().numpy()[0]
                    image2 = (image2 * 255).astype(np.uint8)
                    image2 = image2[0]*0.299 + image2[1]*0.587 + image2[2]*0.114
                    image2 = np.expand_dims(image2, axis=0).astype(np.uint8)

                    # entropy compute
                    H_prob_0 = calcEntropy_1(image1).tolist()[0]
                    H_prob_1 = calcEntropy_1(image2).tolist()[0]
                    ...
                    # Entropy-Aware Regularization-1  
                    # loss = (-(torch.log(torch.sigmoid(config.train.beta*(torch.log(ratio_0)) - config.train.beta*(torch.log(ratio_1)))) + config.train.lamda * (H_prob_0 - H_prob_1))).mean()                    
                    
                    # Entropy-Aware Regularization-2  
                    # loss = (-torch.log(torch.sigmoid(config.train.beta*(torch.log(ratio_0)) - config.train.beta*(torch.log(ratio_1)) + config.train.lamda * (H_prob_0 - H_prob_1)))).mean()
                    
                    # Entropy-Aware Normalization 
                    # loss = -torch.log(torch.sigmoid(config.train.beta*(torch.log(ratio_0))/H_prob_0 - config.train.beta*(torch.log(ratio_1))/H_prob_1)).mean()
                    
                    # EAPA-1
                    # loss = (-(torch.log(torch.sigmoid(config.train.beta*(torch.log(ratio_0))/H_prob_0 - config.train.beta*(torch.log(ratio_1))/H_prob_1)) + config.train.lamda * (H_prob_0 - H_prob_1))).mean()
